...
void mparse_Input1Rx(uint8_t x)
{
    ...
    {
        midiio_RealtimeMsg(MMSG_SOURCE_INPUT1 | MMSG_FLAG_MSG_OK, x);
    }
    else
    {
        bool_t x_used = FALSE;
        ...
        do
        {
            if (Input1State.receiving == FALSE)
            {
                ...
                if (type != MIDI_TYPE_DATA)
                {
                    ...
                    Input1State.msg_index = midiio_MsgNew(MMSG_SOURCE_INPUT1, x);
                    Input1State.midi_status = x;
                    x_used = TRUE;

                    // Predict expected length
                    Input1State.expected_data_count = mmsg_GetDataCount(x);
                ...
                else
                {
                    ...
                    if (Input1State.allow_running_status)
                    {
                        ...
                        Input1State.msg_index = midiio_MsgNew(
                                MMSG_SOURCE_INPUT1 | MMSG_FLAG_RUNNING_STATUS,
                                Input1State.midi_status);

                        // Predict expected length
                        Input1State.expected_data_count = mmsg_GetDataCount(Input1State.midi_status);
                    }
                    else
                    {
                        // We can't do running status, so we don't know what status is!
                        // Add data as raw
                        Input1State.msg_index = midiio_MsgNew(MMSG_SOURCE_INPUT1 | MMSG_FLAG_RAW, 0);
                ...
                {
                    // Yep
                    midiio_MsgFinish(Input1State.msg_index, MMSG_FLAG_MSG_OK);
                    Input1State.receiving = FALSE;
                }
            }
            else
            {
                ...
                {
                    // Yes, store it
                    midiio_MsgAddData(Input1State.msg_index, x);
                ...
                else
                {
                    ...
                    midiio_MsgFinish(Input1State.msg_index, 0u);
                    Input1State.receiving = FALSE;
...
void mparse_Input2Rx(uint8_t x)
{
    ...
    if (type == MIDI_TYPE_SYS_REALTIME)
    {
        midiio_RealtimeMsg(MMSG_SOURCE_INPUT2 | MMSG_FLAG_MSG_OK, x);
    }
    else
    {
        bool_t x_used = FALSE;
        ...
        do
        {
            if (Input2State.receiving == FALSE)
            {
                ...
                {
                    // Yes, we'll use it for status, create new message
                    Input2State.msg_index = midiio_MsgNew(MMSG_SOURCE_INPUT2, x);
                    Input2State.midi_status = x;
                    x_used = TRUE;

                    // Predict expected length
                    Input2State.expected_data_count = mmsg_GetDataCount(x);
                ...
                else
                {
                    ...
                    {
                        // Ok, this is normal running status, create message
                        Input2State.msg_index = midiio_MsgNew(
                                MMSG_SOURCE_INPUT2 | MMSG_FLAG_RUNNING_STATUS,
                                Input2State.midi_status);

                        // Predict expected length
                        Input2State.expected_data_count = mmsg_GetDataCount(Input2State.midi_status);
                    }
                    else
                    {
                        // We can't do running status, so we don't know what status is!
                        // Add data as raw
                        Input2State.msg_index = midiio_MsgNew(MMSG_SOURCE_INPUT2 | MMSG_FLAG_RAW, 0);
                ...
                {
                    // Yep
                    midiio_MsgFinish(Input2State.msg_index, MMSG_FLAG_MSG_OK);
                    Input2State.receiving = FALSE;
                }
            }
            else
            {
                ...
                {
                    // Yes, store it
                    midiio_MsgAddData(Input2State.msg_index, x);
                ...
                else
                {
                    ...
                    // We must finish current message. Do so without the OK flag, since it didn't
                    // have the expected length
                    midiio_MsgFinish(Input2State.msg_index, 0u);
                    Input2State.receiving = FALSE;
